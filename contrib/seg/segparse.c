...
{
       0,    66,    66,    78,    96,   106,   116,   124,   136,   150
};
#endif
...
yy_symbol_value_print (FILE *yyo,
                       yysymbol_kind_t yykind, YYSTYPE const * const yyvaluep, SEG *result, struct Node *escontext)
{
  FILE *yyoutput = yyo;
  YY_USE (yyoutput);
  YY_USE (result);
...
yy_reduce_print (yy_state_t *yyssp, YYSTYPE *yyvsp,
                 int yyrule, SEG *result, struct Node *escontext)
{
...
yydestruct (const char *yymsg,
            yysymbol_kind_t yykind, YYSTYPE *yyvaluep, SEG *result, struct Node *escontext)
{
  YY_USE (yyvaluep);
  YY_USE (result);
  YY_USE (escontext);
...
int
yyparse (SEG *result, struct Node *escontext)
{
...
#line 125 "segparse.y"
		...
		if (!seg_atof((yyvsp[0].text), &val, escontext))
			YYABORT;
